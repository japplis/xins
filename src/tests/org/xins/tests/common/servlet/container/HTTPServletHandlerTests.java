/*
 * $Id: HTTPServletHandlerTests.java,v 1.6 2013/01/22 15:13:22 agoubard Exp $
 *
 ...
 *
 * @version $Revision: 1.6 $ $Date: 2013/01/22 15:13:22 $
...
public class HTTPServletHandlerTests extends TestCase {
   ...
   public void testCacheControl() throws Exception {
      ...
      String     queryString = "/?_convention=_xins-std&_function=FastData&productId=123456789";
      String     host        = AllTests.host();
      int        port        = AllTests.port();
      Properties headers     = null;
      ...
      // Content-length header should be set and correct
      List cacheControlHeaders = result.getHeaderValues("cache-control");
      assertEquals("Expected GET request to return 1 \"Cache-Control\" header. Instead it returned " + cacheControlHeaders.size(), 1, cacheControlHeaders.size());
      String header = (String) cacheControlHeaders.get(0);
      assertEquals("Expected \"Cache-Control\" header from GET request to max-age=60.", "max-age=60", header.toLowerCase().replace(" ", ""));
   }
