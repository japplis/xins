/*
 * $Id: StandardCallingConventionTests.java,v 1.8 2011/01/19 18:13:39 agoubard Exp $
 *
 ...
 *
 * @version $Revision: 1.8 $ $Date: 2011/01/19 18:13:39 $
...
public class StandardCallingConventionTests extends TestCase {
   ...
   public void callResultCodeStandard(String convention) throws Throwable {
      ...
      Element result1 = ElementFormatter.parse(new StringReader(result1Data));
      assertEquals("The method returned an error code for the first call: " + result1.getAttribute("errorcode"), "", result1.getAttribute("errorcode"));
      assertEquals("The method returned a code attribute for the first call: " + result1.getAttribute("code"), "", result1.getAttribute("code"));
      assertEquals("The method returned a success attribute for the first call: " + result1.getAttribute("success"), "", result1.getAttribute("success"));

      String result2Data = CallingConventionTests.callResultCode(convention, randomFive);
      Element result2 = ElementFormatter.parse(new StringReader(result2Data));
      assertNotNull("The method did not return an error code for the second call.", result2.getAttribute("errorcode"));
      assertEquals("The method returned a code attribute for the second call: " + result2.getAttribute("code"), "", result2.getAttribute("code"));
      assertEquals("The method returned a success attribute for the second call: " + result2.getAttribute("success"), "", result2.getAttribute("success"));
   }
}
