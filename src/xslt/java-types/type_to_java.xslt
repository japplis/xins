...
<!--
 ...
 $Id: type_to_java.xslt,v 1.43 2012/03/03 21:23:44 agoubard Exp $
	...
	<xsl:template match="type">
		...
		<xsl:variable name="kind">
			...
			<xsl:choose>
				...
				<xsl:when test="xml">xml</xsl:when>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="superclass">
			<xsl:choose>
				...
				<xsl:when test="$kind = 'xml'">org.xins.common.types.XMLType</xsl:when>
			</xsl:choose>
		</xsl:variable>
      ...
      super("</xsl:text>
		...
		<xsl:choose>
         ...
         new Item("</xsl:text>
			...
			<xsl:when test="$kind = 'xml'">
				<xsl:text>"</xsl:text>
				<xsl:call-template name="xml_to_java_string">
					<xsl:with-param name="text" select="xsd/text()" />
				</xsl:call-template>
				<xsl:text>"</xsl:text>
			</xsl:when>
		</xsl:choose>
