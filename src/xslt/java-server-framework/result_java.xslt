...
<!--
 ...
 $Id: result_java.xslt,v 1.70 2013/01/12 16:37:02 agoubard Exp $
...
public interface UnsuccessfulResult extends Result {
		...
		<!-- ************************************************************* -->
		<!-- Generate the NotModifiedResult class                          -->
		<!-- ************************************************************* -->

		<xsl:text><![CDATA[
/**
 * Not modified result since last call to the <em>]]></xsl:text>
		<xsl:value-of select="@name" />
		<xsl:text><![CDATA[</em> function from the same client.
 */
public final class NotModifiedResult extends org.xins.server.NotModifiedResult implements Result {
}
]]></xsl:text>
...
implements Result {
   ...
   public ]]></xsl:text>
	...
	<xsl:template match="output/data/element | input/data/element" mode="addMethod">
		...
		<!-- First, write in the SuccessfulResult the add(object) method -->
		<xsl:text><![CDATA[
   ...
   public void add</xsl:text>
		...
		<xsl:value-of select="$javaVariable" />
		<xsl:text>) {
      getDataElementBuilder().addToDataElement(</xsl:text>
		<xsl:value-of select="$javaVariable" />
		<xsl:text>.getElement());
   }
</xsl:text>
   ...
   public static final class </xsl:text>
       ...
       */
      private final org.w3c.dom.Element _element = org.xins.common.xml.ElementFormatter.createMainElement("</xsl:text>
		<xsl:value-of select="@name" />
		<xsl:text><![CDATA[");
       ...
       */
      final org.w3c.dom.Element getElement() {
         return _element;
      }
   ...
   public void add</xsl:text>
		...
		<xsl:value-of select="$javaVariable" />
		<xsl:text>) {
      _element.appendChild(_element.getOwnerDocument().importNode(</xsl:text>
		<xsl:value-of select="$javaVariable" />
		<xsl:text>.getElement(), true));
   }
</xsl:text>
	</xsl:template>
