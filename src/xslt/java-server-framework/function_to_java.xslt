...
<!--
 ...
 $Id: function_to_java.xslt,v 1.65 2013/01/12 16:37:02 agoubard Exp $
...
public abstract class ]]></xsl:text>
		...
		<xsl:if test="$incl_shared != 'false'">
			<xsl:for-each select="$impl_node/instance">
				<xsl:text>
   ...
   throws Throwable {</xsl:text>
		...
		<xsl:if test="input/data/element">
			<xsl:text>
      org.w3c.dom.Element _dataElement = _context.getDataElement();</xsl:text>
		</xsl:if>
		...
		<xsl:text>
      Request _request = new Request(_context.getBackpack()</xsl:text>
      ...
      if (!(_result instanceof SuccessfulResult) &amp;&amp; !(_result instanceof NotModifiedResult)</xsl:text>
			...
			<xsl:text>) {
         throw org.xins.common.Utils.logProgrammingError("The result is not a successful result or a not modified result or a defined error code: " +
