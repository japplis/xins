...
<!--
 ...
 $Id: xins-project_to_ant-build.xslt,v 1.371 2012/10/20 08:17:48 agoubard Exp $
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="api">
		...
		<xsl:for-each select="impl">
			...
			<target name="war-{$api}{$implName2}" depends="-load-properties-{$api}, classes-api-{$api}{$implName2}, wsdl-{$api}" description="Creates the WAR for the '{$api}{$implName2}' API" unless="no-war-{$api}{$implName2}">
				...
				duplicate="fail">
					...
					<lib dir="{$xins_home}/lib"   includes="commons-codec.jar httpclient.jar httpcore.jar commons-logging.jar log4j.jar xmlenc.jar json.jar logdoc.jar znerd-util.jar" />
		...
		<target name="mock-{$api}" depends="-load-properties-{$api}, capi-{$api}" description="Generates an mock CAPI using the defined examples">
			<path id="classes.api.classpath">
				<xsl:if test="$apiHasTypes">
					<pathelement path="{$typeClassesDir}" />
				</xsl:if>
		...
		<target name="client-{$api}"
						depends="jar-{$api}, javadoc-capi-{$api}, specdocs-{$api}, wsdl-{$api}, opendoc-{$api}, mock-{$api}"
						description="Put documentation and client implementation for '{$api}' in a zip file.">
