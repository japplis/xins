...
<!--
 ...
 $Id: types.xslt,v 1.63 2012/03/03 21:23:44 agoubard Exp $
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:text="urn:oasis:names:tc:opendocument:xmlns:text:1.0" xmlns:xd="http://www.pnp-software.com/XSLTdoc">
	...
	<xd:doc type="stylesheet">
		...
		<xd:copyright>Copyright 2003-2008 Online Breedband B.V.</xd:copyright>
		<xd:cvsId>$Id: types.xslt,v 1.63 2012/03/03 21:23:44 agoubard Exp $</xd:cvsId>
	</xd:doc>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:choose>
					...
					<xsl:when test="$type_node/xml">
						<xsl:text>_xml</xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:text>_text</xsl:text>
					</xsl:otherwise>
	...
	*    'Boolean', 'java.util.Map<String, String>',
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<!-- TODO the other cases ($type_node/properties, $type_node/list, $type_node/xml ...) -->
					</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
		</xsl:choose>
