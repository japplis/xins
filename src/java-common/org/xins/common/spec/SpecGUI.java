/*
 * $Id: SpecGUI.java,v 1.15 2012/03/15 21:07:39 agoubard Exp $
 *
 ...
 *
 * @version $Revision: 1.15 $ $Date: 2012/03/15 21:07:39 $
...
public class SpecGUI {
   ...
   protected void initData() {
      try {
         Element webapp = ElementFormatter.parse(getClass().getResourceAsStream("/WEB-INF/web.xml"));
         String apiClassName = null;
         Element servletElement = new ElementList(webapp, "servlet").getUniqueChildElement();
         for (Element initParam : new ElementList(servletElement, "init-param")) {
            String paramName = new ElementList(initParam, "param-name").getUniqueChildElement().getTextContent();
            if (paramName.equals("org.xins.api.class")) {
               apiClassName = new ElementList(initParam, "param-value").getUniqueChildElement().getTextContent();
            }
         }
   ...
   protected void createMenuBar() {
      ...
      try {
         Element api = ElementFormatter.parse(getClass().getResourceAsStream("/WEB-INF/specs/api.xml"));
         ...
         ElementList types = new ElementList(api, "type");
         if (!types.isEmpty()) {
            specMenu.addSeparator();
         }
         for (Element nextType : types) {
         ...
         ElementList resultCodes = new ElementList(api, "resultcode");
         if (!resultCodes.isEmpty()) {
            specMenu.addSeparator();
         }
         for (Element nextResultCode : resultCodes) {
            ...
            public void changedUpdate(DocumentEvent de) {
               try {
                  ...
                  try {
                     ...
                     jtfPattern.setBackground(tfValidColor);
                     if (pattern.matcher(jtfTextToTest.getText()).matches()) {
